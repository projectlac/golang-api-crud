...
func Register(w http.ResponseWriter, r *http.Request) {
	...
	db := createConnection()
	defer db.Close()
	// sqlStatement := `SELECT * FROM auth WHERE username='$1'`
	// errCheckDuplicate := db.QueryRow(sqlStatement, username).Err()

	// if errCheckDuplicate == nil {
	// 	res.JSON(w, 409, "Trung")
	// 	return
	// }
	...
	// unmarshal the row object to user
	errCheckDuplicate := row.Scan(&auth.ID, &auth.Username, &auth.Password, &auth.Email)

	if errCheckDuplicate == nil {
		res.JSON(w, 409, "User does exist")
		return
	}
	...
	sql := `INSERT INTO auth (username, email, password) VALUES ($1, $2, $3) RETURNING id`
	var id int64
	insertErr := db.QueryRow(sql, username, email, password).Scan(&id)
	if insertErr != nil {
		log.Fatalf("Unable to execute the query. %v", err)
	}
...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	if errCheckDuplicate != nil {
		res.JSON(w, 409, "Username or Password incorrect")
		return
	}

	// convert interface to string
	hashedPassword := fmt.Sprintf("%v", auth.Password)
